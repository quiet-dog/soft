  ...
  <div class="w-full">
    ...
    <a-modal v-model:visible="visible" width="800px" draggable :footer="false">
      ...
      <a-tabs class="tabs">
        ...
        <a-tab-pane key="devinggo" title="devinggoAdmin">
          <ul class="devinggo">
            <li v-for="icon in devinggoadminIcons" :key="icon" @click="selectIcon(icon, 'devinggo')">
              <component :is="icon" />
            </li>
          </ul>
        </a-tab-pane>
...
<script setup>
  ...
  const devinggoadminIcons = reactive([])
  ...
  for (const path in modules) {
    ...
    devinggoadminIcons.push(`MaIcon${name}`)
  }
