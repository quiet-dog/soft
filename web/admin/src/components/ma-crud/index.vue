  ...
  <a-layout-content class="flex flex-col lg:h-full relative w-full">
    <div class="_crud-header flex flex-col mb-2" ref="crudHeaderRef">
      <a-tabs v-if="isArray(options.tabs.data) && options.tabs.data.length > 0"
        ...
        :hide-content="true" @change="tabChange" @tab-click="runEvent(options.tabs, 'onClick', undefined, $event)"
        class="ma-tabs mb-5">
        <template #extra>
      ...
      <ma-search @search="searchSubmitHandler" class="__search-panel" ref="crudSearchRef">
        <template v-for="(slot, slotIndex) in getSearchSlot()" :key="slotIndex" #[slot]="{ searchForm, component }">
      ...
      <div v-if="$slots.middleContent" class="mb-2">
    ...
    <div class="_crud-content">
      <div class="operation-tools lg:flex justify-between mb-3" ref="crudOperationRef">
        <a-space class="lg:flex block lg:inline-block">
          ...
          <slot name="tableButtons">
            <a-button v-if="options.add.show" v-auth="options.add.auth || []" v-role="options.add.role || []"
              @click="addAction" type="primary" class="w-full lg:w-auto mt-2 lg:mt-0">
            ...
            <a-popconfirm content="确定要删除数据吗?" position="bottom" @ok="deletesMultipleAction"
              ...
              <a-button v-auth="options.delete.auth || []" v-role="options.delete.role || []" type="primary"
                status="danger" class="w-full lg:w-auto mt-2 lg:mt-0">
            ...
            <a-popconfirm content="确定要恢复数据吗?" position="bottom" @ok="recoverysMultipleAction"
              ...
              <a-button v-auth="options.recovery.auth || []" v-role="options.recovery.role || []" type="primary"
                status="success" class="w-full lg:w-auto mt-2 lg:mt-0">
                <template #icon><icon-undo /></template>{{ options.recovery.text || '恢复' }}</a-button>
            </a-popconfirm>

            <a-button v-if="options.import.show" v-auth="options.import.auth || []" v-role="options.import.role || []"
              @click="importAction" class="w-full lg:w-auto mt-2 lg:mt-0"><template #icon><icon-upload /></template>{{
                options.import.text || '导入' }}</a-button>

            <a-button v-if="options.export.show" v-auth="options.export.auth || []" v-role="options.export.role || []"
              @click="exportAction" class="w-full lg:w-auto mt-2 lg:mt-0"><template #icon><icon-download /></template>{{
        ...
        <a-space class="lg:mt-0 mt-2" v-if="options.showTools">
          ...
          <a-tooltip :content="isRecovery ? '显示正常数据' : '显示回收站数据'"
      ...
      <div ref="crudContentRef">
        <slot name="crudContent" v-bind="tableData">
          <a-table v-if="tableIsShow" v-bind="$attrs" ref="tableRef" :key="options.pk" :data="tableData"
            :loading="loading" :sticky-header="options.stickyHeader" :pagination="options.tablePagination"
            :stripe="options.stripe" :bordered="options.bordered" :rowSelection="options.rowSelection ?? undefined"
            ...
            :summary="(options.customerSummary || options.showSummary) && __summary" v-model:selectedKeys="selecteds"
            @sorter-change="handlerSort" @select="onSelect">
            <template #tr="{ record }">
              <tr class="ma-crud-table-tr"
                ...
                @contextmenu.prevent="openContextMenu($event, record)" @dblclick="dbClickOpenEdit(record)" />
            </template>
            ...
            <template #columns>
              <ma-column ref="crudColumnRef" v-if="reloadColumn" :columns="columns" :isRecovery="isRecovery"
                :crudFormRef="crudFormRef" @refresh="() => refresh()" @showImage="showImage">
                ...
                <template v-for="(slot, slotIndex) in getSlot(columns)" :key="slotIndex"
    ...
    <div class="_crud-footer mt-3 text-right" ref="crudFooterRef"
      ...
      <a-pagination :total="total" show-total show-jumper show-page-size :page-size-options="options.pageSizeOption"
        @page-size-change="pageSizeChangeHandler" @change="pageChangeHandler"
        v-model:current="requestParams[config.request.page]" :page-size="requestParams[config.request.pageSize]"
...
// 处理选中状态变化
const onSelect = (rowKeys,rowKey,row) => {
  if (row.children) {
    if (rowKeys.includes(rowKey)) {
      row.children.forEach(child => {
        if (!rowKeys.includes(child.id)) {
          rowKeys.push(child.id);
        }
      });
    } else {
      row.children.forEach(child => {
        const index = rowKeys.indexOf(child.id);
        if (index > -1) {
