  ...
  <a-drawer class="backend-setting" v-model:visible="visible" :on-before-ok="save" width="350px"
    ...
    <a-form :model="form" :auto-label-width="true">
      <a-row class="flex justify-center mb-5">
        ...
        <ColorPicker theme="dark" :color="appStore.color" :sucker-hide="true" :colors-default="defaultColorList"
          @changeColor="changeColor" style="width: 218px;" />
      </a-row>
      ...
      <a-form-item :label="$t('sys.lockScreenPwd')" :help="$t('sys.lockScreenPwdHelp')">
        <a-input v-model="form.lockScreenPwd" type="text" @change="handleLockScreenPwd" />
      </a-form-item>
      ...
      <a-form-item :label="$t('sys.dark')" :help="$t('sys.darkHelp')" v-if="currentSkin === 'devinggo'">
...
const handleLockScreenPwd = (val) => appStore.changeLockScreenPwd(val)
...
const save = async (done) => {
  const data = {
    ...
    ws: appStore.ws,
    lockScreenPwd: appStore.lockScreenPwd
  }
