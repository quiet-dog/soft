<template>
  <a-modal v-model:visible="visible" width="600px" @cancel="close" :footer="false">
    ...
    <div class="flex flex-col">
      <a-card v-for="(item, index) in skinList" :key="item.name" :class="index === 0 ? '' : 'mt-3'"
        ...
        <a-row class="w-full flex items-center">
          ...
          <a-col :span="3" class="flex items-center justify-end">
            <a-button :type="appStore.skin === item.name ? 'primary' : 'secondary'"
              :disabled="appStore.skin === item.name" @click="useSkin(item.name)">
