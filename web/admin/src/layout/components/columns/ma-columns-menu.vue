<template>
    ...
    <ul class="mt-1 parent-menu-container">
      <template v-for="(bigMenu, index) in userStore.routers" :key="index">
        <li :class="`${classStyle}`" @click="() => {
          ...
          <span class="mt-0.5" :style="appStore.language === 'en' ? 'font-size: 10px' : ''">{{ appStore.i18n ? (
            $t(`menus.${bigMenu.name}`).indexOf('.') > 0 ? bigMenu.meta.title : $t(`menus.${bigMenu.name}`)) :
            bigMenu.meta.title }}</span>
        </li>
      </template>
    </ul>
  </div>
  <div class="layout-menu shadow flex flex-col" v-show="showMenu">
    ...
    <a-layout-sider :style="`width: ${appStore.menuCollapse ? '50px' : appStore.menuWidth + 'px'};
