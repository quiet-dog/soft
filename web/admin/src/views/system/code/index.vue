  ...
  <div class="ma-content-block lg:flex justify-between p-4">
    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef" @selection-change="selectionChange">
      ...
      <template #tableAfterButtons>
        <a-button v-auth="['system:code:generate']" type="outline" @click="batchGenerate"><template
            #icon><icon-code /></template>生成代码</a-button>
        <a-button v-auth="['system:code:loadTable']" @click="() => loadTableRef.open()" type="outline"
          status="success"><template #icon><icon-export /></template>装载数据表</a-button>
      </template>
      <!-- 操作前置扩展 -->
      <template #operationBeforeExtend="{ record }">
        ...
        <a-link v-auth="['system:code:generate']" @click="generateCode(record.id)"><icon-code /> 生成代码</a-link>
      </template>
    </ma-crud>
...
<script setup>
  ...
  const crud = reactive({
    ...
    delete: {
      show: true,
      api: generate.deletes, auth: ['system:code:delete'],
    },
  })
...
<script>
export default { name: 'system:code' }
</script>
