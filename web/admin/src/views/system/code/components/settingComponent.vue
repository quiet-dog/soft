  ...
  <a-modal v-model:visible="visible" :on-before-ok="save" width="600px" draggable top="50px" :align-center="false">
    ...
    <a-form :model="form">
      ...
      <div v-if="row.view_type == 'inputNumber' || row.view_type == 'slider'">
        <a-form-item label="最小值" field="min" label-col-flex="auto" :label-col-style="{ width: '80px' }">
          <a-input-number v-model="form.min" />
        </a-form-item>
        <a-form-item label="最大值" field="max" label-col-flex="auto" :label-col-style="{ width: '80px' }">
          <a-input-number v-model="form.max" />
        </a-form-item>
        <a-form-item label="步长" field="step" label-col-flex="auto" :label-col-style="{ width: '80px' }">
          <a-input-number v-model="form.step" :min="0" />
        </a-form-item>
        <a-form-item label="精度" field="precision" label-col-flex="auto" :label-col-style="{ width: '80px' }"
          v-if="row.view_type != 'slider'">
          <a-input-number v-model="form.precision" :min="0" />
        </a-form-item>
        <a-form-item label="刻度线" field="showTicks" label-col-flex="auto" :label-col-style="{ width: '80px' }"
      ...
      <div v-if="row.view_type == 'switch'">
        <a-form-item label="选中时的值" field="checkedValue" label-col-flex="auto" :label-col-style="{ width: '120px' }">
          <a-input v-model="form.checkedValue" />
        </a-form-item>
        <a-form-item label="未选中时的值" field="uncheckedValue" label-col-flex="auto" :label-col-style="{ width: '120px' }">
          <a-input v-model="form.uncheckedValue" />
        </a-form-item>
      </div>
      <!-- 下拉、复选、单选 -->
      <div v-if="['select', 'checkbox', 'radio', 'transfer'].includes(row.view_type)">
        <a-form-item label="是否多选" field="multiple" label-col-flex="auto" :label-col-style="{ width: '80px' }"
        ...
        <a-card v-for="(item, idx) in form.collection" :key="idx" class="mt-2 relative">
          ...
          <a-form-item label="名称" field="label" label-col-flex="auto" :label-col-style="{ width: '80px' }">
            <a-input v-model="item.label" />
          </a-form-item>
          <a-form-item label="值" field="value" label-col-flex="auto" :label-col-style="{ width: '80px' }">
      ...
      <div v-if="['codeEditor', 'editor', 'wangEditor'].includes(row.view_type)">
        ...
        <a-form-item label="双向绑定数据" field="isBind" label-col-flex="auto" :label-col-style="{ width: '120px' }"
      ...
      <div v-if="['upload', 'selectResource'].includes(row.view_type)">
        <a-form-item label="返回数据" field="onlyData" label-col-flex="auto" :label-col-style="{ width: '120px' }">
          <a-radio-group v-model="form.onlyData">
            <a-radio :value="true">单个字段数据</a-radio>
            <a-radio :value="false">全量数据</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item label="返回数据" field="returnType" label-col-flex="auto" :label-col-style="{ width: '120px' }"
          v-if="form.onlyData" :extra="`支持 uploadfile 数据表所有字段，这里仅列常用部分`">
        ...
        <a-form-item label="是否可多选" field="multiple" label-col-flex="auto" :label-col-style="{ width: '120px' }">
          <a-radio-group v-model="form.multiple">
            <a-radio :value="true">是</a-radio>
            <a-radio :value="false">否</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item label="是否分片上传" field="chunk" label-col-flex="auto" :label-col-style="{ width: '120px' }"
          extra="分片上传不限制文件类型，选择分片上传后，上传文件类型则失效" v-if="row.view_type == 'upload'">
          <a-radio-group v-model="form.chunk">
            <a-radio :value="true">是</a-radio>
            <a-radio :value="false">否</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item label="上传类型" field="type" label-col-flex="auto" :label-col-style="{ width: '100px' }"
          v-if="row.view_type == 'upload' && !form.chunk" :extra="`可在 系统配置 -> 上传配置 里修改允许格式`">
      ...
      <div v-if="row.view_type == 'selectUser'">
        <a-form-item label="返回数据" field="onlyId" label-col-flex="auto" :label-col-style="{ width: '100px' }">
      ...
      <div v-if="['time', 'date'].includes(row.view_type)">
        ...
        <a-form-item class="mt-3" label="是否范围选择" field="range" label-col-flex="auto"
