      ...
      <ma-crud :options="crud" :columns="columns" ref="crudRef">
        ...
        <template #status="{ record }">
          <div v-if="record.id == 1">
            <a-switch disabled :checked-value="1" unchecked-value="2" @change="changeStatus($event, record.id)"
        ...
        <template #avatar="{ record }">
          <a-avatar>
            <img :src="record.avatar ? $tool.showFile(record.avatar) : $url + 'avatar.png'" style="object-fit: cover" />
          </a-avatar>
        </template>
        <!-- 操作列 -->
        ...
        <template #operationAfterExtend="{ record }">
          <a-dropdown trigger="hover" v-if="record.id != 1 && !isRecovery"
...
<script setup>
  ...
  const crud = reactive({
    ...
    formOption: {
      ...
      layout: [
        ...
        { formType: 'grid', cols: [ { span: 24, formList: [ { dataIndex: 'user_type' }] }] },
  ...
  const columns = reactive([
    ...
    {
      title: '用户类型', dataIndex: 'user_type', width: 100, search: true, formType: 'select',
      dict: { name: 'user_type', props: { label: 'title', value: 'key' },translation: true, },
      commonRules: [{ required: true, message: '用户类型必选' }],
