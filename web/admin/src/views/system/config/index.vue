<template>
  <div class="lg:flex justify-between">
    <div class="ma-content-block p-5 h-full lg:w-1/2">
      <a-tabs type="rounded" v-model:active-key="active" @delete="openDeleteModal" @change="handleChange"
        :updateable="true" auto-switch :editable="true">
        <template #extra>
          <a-space>
        ...
        <a-tab-pane v-for="(item, index) in configGroupData" :key="`${index}-${item.id}`" :title="item.name">
          <ma-form v-if="isCreateNode" v-model="formArray[item.id]" :columns="optionsArray[item.id]" @submit="submit"
            class="mt-3" ref="maFormRef" />
        </a-tab-pane>
      </a-tabs>
    </div>
