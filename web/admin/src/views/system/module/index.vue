  ...
  <div class="ma-content-block lg:flex justify-between p-4">
    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef">
      <template #operationCell="{ record }">
        ...
        <a-popconfirm @ok="handleInstall(record)" :content="`确定安装模块数据？如表和数据已存在则可能无效！`" position="bottom">
          <a-link v-auth="['setting:module:install']"><icon-record /> 安装数据</a-link>
        </a-popconfirm>

        <a-popconfirm @ok="switchEnbaled(record)" :content="`确定要${record.enabled ? '停用' : '启用'}${record.label}吗?`"
          position="bottom">
        ...
        <a-link @click="openDeleteModal(record)" v-auth="['setting:module:delete']"><icon-delete /> 删除</a-link>
      </template>
    </ma-crud>
...
<script setup>
  ...
  import module from '@/api/system/module'
