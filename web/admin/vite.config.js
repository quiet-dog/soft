...
export default ({ mode }) => {
  ...
  const asyncComponentPrefix = env.VITE_ASYNC_COMPONET_PROXY_PREFIX;
  ...
  return defineConfig({
    ...
    server: {
      ...
      proxy: {
        ...
        [asyncComponentPrefix]: {
          target: env.VITE_ASYNC_COMPONET_BASE_URL,
          changeOrigin: true,
          ws: true,
          toProxy: true,
          rewrite: (path) => path.replace(new RegExp(`^${asyncComponentPrefix}`), ''),
        }
      },
      cors: {
